...
function _dist_params_numtype(d::Distribution)
    promote_type(map(typeof, Distributions.params(d))...)
end

@inline function _trafo_cdf(d::Distribution{Univariate,Continuous}, x::Real)
    _trafo_cdf_impl(_dist_params_numtype(d), d, x)
end

@inline function _trafo_cdf_impl(
    ::Type{<:Real},
...
@inline function _trafo_quantile(d::Distribution{Univariate,Continuous}, u::Real)
    _trafo_quantile_impl(_dist_params_numtype(d), d, u)
end

@inline function _trafo_quantile_impl(
    ::Type{<:Real},
    d::Distribution{Univariate,Continuous},
    u::Real,
)
...
@inline function _trafo_quantile_impl_generic(
    d::Distribution{Univariate,Continuous},
    u::Real,
)
...
@inline function MeasureBase.transport_def(
    ...
    if 0 <= x <= 1
        ...
        mod_x = ifelse(
            x == 0,
            zero(TF) + eps(TF),
            ifelse(x == 1, one(TF) - eps(TF), convert(TF, x)),
        )
...
for (A, B) in [(Uniform, StdUniform), (Logistic, StdLogistic), (Normal, StdNormal)]
    @eval begin
...
function MeasureBase.transport_def(
    ν::Distribution{Univariate},
    μ::PowerMeasure{<:StdMeasure},
    x,
)
