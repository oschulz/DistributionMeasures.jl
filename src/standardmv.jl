...
@inline function MeasureBase.vartransform_def(trg::StandardDist{Uniform,N}, src::StandardDist{Normal,N}, x) where N
    _check_arraylike_match(trg, src, x)
    StatsFuns.normcdf.(x)
end


@inline function MeasureBase.vartransform_def(trg::StandardDist{Normal,N}, src::StandardDist{Uniform,N}, x) where N
    _check_arraylike_match(trg, src, x)
